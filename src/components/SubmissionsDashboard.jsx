...
const SubmissionsDashboard = () => {
    ...
    const hasAttachment = (sub) =>
        sub['Attachment URL'] && sub['Attachment URL'].toString().startsWith('http');

    const isImage = (type = '') => type.startsWith('image/');

    const getFileIcon = (type = '') => {
        if (type === 'application/pdf')  return { icon: 'icofont-file-pdf',      color: '#ef4444', label: 'PDF'  };
        if (type?.includes('word'))      return { icon: 'icofont-file-word',     color: '#2563eb', label: 'DOC'  };
        if (type?.startsWith('image/')) return { icon: 'icofont-file-image',    color: '#059669', label: 'IMG'  };
        return                                  { icon: 'icofont-file-document', color: '#6b7280', label: 'FILE' };
    };

    // Google Drive download URL → viewable image URL
    const toViewURL = (url = '') =>
        url.replace('&export=download', '').replace('export=download&', '')
           .replace('/uc?', '/thumbnail?sz=w400&');

    const AttachmentPreview = ({ sub, inModal = false }) => {
        if (!hasAttachment(sub)) {
            return (
        ...
        const fileURL = sub['Attachment URL'];
        const name    = sub['Attachment Name'];

        return (
            <div style={{ ...styles.attachmentBox, ...(inModal ? { marginTop: 12 } : {}) }}>
                {isImage(sub['Attachment Type']) ? (
                    <div style={inModal ? {} : styles.imageThumbWrap}>
                        <img
                            src={toViewURL(fileURL)}
                            alt={name}
                            style={inModal ? styles.modalImage : styles.imageThumb}
                            onError={e => { e.target.style.display = 'none'; }}
                        />
                        {!inModal && (
                            <div style={styles.imageThumbOverlay}>
                    ...
                    <div style={styles.docPreview}>
                        <i className={att.icon} style={{ fontSize: inModal ? 36 : 28, color: att.color, flexShrink: 0 }}></i>
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={styles.attachName}>{name}</div>
                ...
                <a
                    ...
                    style={{ ...styles.downloadBtn, ...(inModal ? { width: '100%', justifyContent: 'center', marginTop: 12, display: 'flex' } : { display: 'inline-flex' }) }}
                >
                    <i className="icofont-download" style={{ marginRight: 8 }}></i>
                    {inModal ? `Download ${name}` : 'Download'}
                </a>
            </div>
        );
    };
                    ...
                    <div style={styles.modal} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={styles.modalBody}>
                            ...
                            <div style={{ marginTop: 20 }}>
                                ...
                                <AttachmentPreview sub={selected} inModal={true} />
                            </div>
                        </div>
                    </div>
                </div>
...
const styles = {
    ...
    imageThumbWrap:  { position: 'relative', width: '100%', height: 140, borderRadius: 8, overflow: 'hidden', marginBottom: 10 },
    imageThumb:      { width: '100%', height: '100%', objectFit: 'cover' },
    modalImage:      { width: '100%', borderRadius: 10, maxHeight: 300, objectFit: 'contain', background: '#f9fafb' },
    imageThumbOverlay: { position: 'absolute', inset: 0, background: 'rgba(19,30,60,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' },
    docPreview:      { display: 'flex', alignItems: 'center', gap: 14, paddingBottom: 10 },
    ...
    downloadBtn:     { background: ORANGE, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer', textDecoration: 'none', alignItems: 'center' },
